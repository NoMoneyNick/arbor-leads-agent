...
app = FastAPI(title="Vector Data Labs - V43.0 Discovery Master", docs_url="/docs")
...
# --- DATA ARCHITECTURE (V43.0 Verified) ---
COUNCILS = {
    "Leeds_City_Control": {
        "type": "arcgis",
    ...
    "Southwark_City_Control": {
        "type": "arcgis",
        # Targeting Layer 0 which is 'Planning Applications - Current'
        "url": "https://geo.southwark.gov.uk/arcgis/rest/services/Planning/MapServer/0/query",
        "referer": "https://www.southwark.gov.uk/planning-and-building-control/"
    },
    "London_Official_Hub": {
        "type": "ckan",
        "url": "https://data.london.gov.uk/api/3/action/datastore_search",
        # Note: Resource ID 847f... is currently returning 410 (Gone). 
        # Update this ID below once the GLA provides the new 'Room Number'.
...
CABINET_HEADERS = ["proposal", "description", "development_description", "nature", "details", "PROPOSAL", "PROPOSAL_DETAILS"]
...
def fetch_council(name, config):
    ...
    headers = {
        "User-Agent": "VectorDataLabs/43.0 (Professional Data Integration)",
    ...
    try:
        if config["type"] == "ckan":
            ...
            res = session.get(config["url"], params=params, headers=headers, timeout=20)
            if res.status_code == 200:
                return res.json().get("result", {}).get("records", []), "Online"
            if res.status_code == 410:
                return [], "Error 410: Data has moved (Update Resource ID)"
            ...
            # Timeout set to 15s to prevent the app hanging on blocked servers
            res = session.get(config["url"], params=query_params, headers=headers, timeout=15, verify=False)
            if res.status_code == 200:
                return [f.get("attributes", {}) for f in res.json().get("features", [])], "Online"
            return [], f"HTTP {res.status_code}"

    except Exception as e:
        return [], f"Blocked/Timeout"
    return [], "Offline"
...
def lander():
    ...
    <div style='display:inline-block; padding:50px; background:white; border-radius:15px; box-shadow:0 10px 30px rgba(0,0,0,0.1); border-top: 6px solid #1b5e20; max-width:600px;'>
        ...
        <p>Integration Hub V43.0</p>
        <div style='background:#f1f8e9; padding:15px; border-radius:10px; margin:20px 0; text-align:left; font-size:14px;'>
            <b>Active:</b> Leeds, Southwark<br/>
            <b>Blocked:</b> Manchester (IP Filter Active)<br/>
            <b>Relocated:</b> London GLA (Error 410)
        </div>
