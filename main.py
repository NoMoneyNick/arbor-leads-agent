...
app = FastAPI(title="Vector Data Labs - V22.0 Intelligence Master", docs_url="/docs")
...
# --- THE MASTER CRAWL LIST (Roots Only) ---
# We provide the building root. The engine finds the correct room (Service) and desk (Layer).
COUNCILS = {
    "Leeds_Control": {
        "type": "static",
...
def get_d(r):
    # Dynamic date field search
    v = r.get("DATE_RECEIVED") or r.get("DATE_VALID") or r.get("DATEAPVAL") or r.get("RECDAT") or 0
    try: return float(v)
    except: return 0

def classify(r):
    # Dynamic field mapping (Checks all common ArcGIS field names)
    ...
    # Negative filtering for house extensions
    if any(w in p for w in SKIP_WORDS) and score < 8: return False, 0
    return (score > 2), score

# --- THE INTELLIGENCE ENGINE (Probe & Heal) ---
def fetch_council(name, config):
    ...
    # Track 2: Intelligence Probe (The Self-Healing Way)
    if config["type"] == "probe":
        try:
            # Step A: Get the server directory
            catalog_res = session.get(f"{config['root']}?f=json", headers=h, timeout=15, verify=False)
            if "html" in catalog_res.text.lower(): return [], "Blocked by Firewall (HTML)"
            
            services = catalog_res.json().get("services", [])
            # Step B: Find service candidates
            candidates = [s for s in services if any(k in s['name'].lower() for k in ["planning", "register", "development"])]
            
            for s in candidates:
                s_name = s['name']
                s_type = s['type'] # MapServer or FeatureServer
                
                # Step C: Layer Probing (UK Standard Layers)
                for l_id in [0, 5, 12, 1, 2]:
                    probe_url = f"{config['root']}/{s_name}/{s_type}/{l_id}/query"
                    try:
                        probe_res = session.get(probe_url, params=q, headers=h, timeout=10, verify=False)
                        if probe_res.status_code == 200 and "features" in probe_res.text:
                            features = probe_res.json().get("features", [])
                            if len(features) > 0:
                                # Validate this layer has planning descriptions
                                sample = features[0].get("attributes", {})
                                if any(field in sample for field in ["PROPOSAL", "DESCRIPTION", "development_description"]):
...
def lander():
    ...
    <a href='/test-regional' style='color:#1b5e20; text-decoration:none; font-weight:bold;'>Run Master Diagnostic Scan</a>
    </div>
    </body></html>
    """
